...
class Search(models.Model):
    depart_city = models.CharField(
        max_length=50, verbose_name="Город вылета", help_text="Введите город вылета"
    )
    dest_city = models.CharField(
        max_length=50,
        verbose_name="Город назначения",
        help_text="Введите город назначения",
    )
    max_transhipments = models.SmallIntegerField(
        verbose_name="Количество пересадок",
        help_text="Количество пересадок, не более 3",
    )
    depart_date = models.DateField(
        verbose_name="Дата вылета", help_text="Введите дату вылета"
    )
    ...
    num_adults = models.SmallIntegerField(
        verbose_name="Количество взрослых пассажиров", help_text="Не более 10"
    )
    num_children = models.SmallIntegerField(
        null=True, verbose_name="Количество детей", help_text="Не более 5"
    )
    ...
    luggage = models.BooleanField(
        default=False, verbose_name="Багаж", help_text="Багаж"
    )
    search_init_date = models.DateTimeField(
        auto_now_add=True,
        verbose_name="Дата добавления поиска",
        help_text="Дата добавления поиска",
    )
    telegr_acc = models.CharField(
        max_length=50,
...
class SearchArchive(models.Model):
    old_id = models.IntegerField(verbose_name="Старый ID")
    depart_city = models.CharField(
        max_length=50, verbose_name="Город вылета", help_text="Введите город вылета"
    )
    dest_city = models.CharField(
    ...
    def __str__(self):
        return f"archive flight {self.depart_city} - {self.dest_city}"
